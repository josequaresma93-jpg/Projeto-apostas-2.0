...
st.title('🔍 Scanner por Jogo')
leagues=get_leagues(); league_options=['Série B']+[x for x in leagues.name.tolist() if x!='Série B']
...
confidence=context_confidence(hs,aw,manual_conf); explanation=explain_context(hs,aw,lh,la)

m1,m2,m3,m4,m5,m6=st.columns(6)
m1.metric('Lambda Casa',f'{lh:.2f}'); m2.metric('Lambda Fora',f'{la:.2f}'); m3.metric('Casa BTTS',f"{hs['btts']:.0%}"); m4.metric('Fora BTTS',f"{aw['btts']:.0%}"); m5.metric('Confiança',f'{confidence:.0f}/100'); m6.metric('Forma média',f"{(hs['form']+aw['form'])/2:.0f}")
with st.expander('Explicação'): st.write(explanation)
...
for i,market in enumerate(main_markets):
    with cols[i%5]:
        ...
        if odd>0: rows.append({'bookmaker':'Manual','market':market,'odd':odd})
...
st.subheader('Radar')
if radar.empty: st.info('Digite odds acima de 0.')
else:
    st.dataframe(radar[['bookmaker','market','prob','fair_odd','odd','edge','ev','kelly','stake','confidence','grade','decision']].style.format({'prob':'{:.2%}','fair_odd':'{:.2f}','odd':'{:.2f}','edge':'{:.2%}','ev':'{:.2%}','kelly':'{:.2%}','stake':'R$ {:.2f}','confidence':'{:.0f}'}),use_container_width=True)
    labels=[f'{r.bookmaker} | {r.market} | odd {r.odd}' for _,r in radar.iterrows()]
    selected=st.selectbox('Entrada',labels); row=radar.iloc[labels.index(selected)]
